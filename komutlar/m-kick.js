...
exports.run = async(client, message, args) => {
	...
	if(!message.member.roles.cache.has(rol)&& !message.member.hasPermission("KICK_MEMBERS")) return message.channel.send('Ban yetkili rolü ayarlanmamış veya <@&' + rol + '> Rolüne sahip değilsin.')
    ...
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     if(!user) return message.channel.send(' ``⚠️Bir kişi etiketlemelisin.``')
     if(user.id === message.author.id) return message.channel.send('``⚠️Kendini Kickleyemezsin``')
     if(user.id === client.user.id) return message.channel.send('``⚠️Botu Kickleyemezsin``')
  if(user.id === message.guild.ownerID) return message.channel.send('``⚠️Sunucu sahibini Kickleyemessin Zaten Yetkim Yetmez :)``')
