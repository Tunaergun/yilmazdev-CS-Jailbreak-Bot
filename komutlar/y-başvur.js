...
exports.run = async (client, message, args) => { 
  ...
  const sorular = [
    ...
    '**Şuanda Aktif Olarak Kaç Sunucuda Yetkilisiniz?** [0/50]',
    `**Neden ${isim}?** <cevabınız>`
  ]
  let sayac = 0
  ...
  collector.on('end', collected => {
    ...
    .addField('Başvuran Hakkında',[
	  ...
	  `**Şuanda Aktif Olarak Kaç Sunucuda Yetkilisiniz?: **\t\t${collected.map(m => m.content).slice(3,4)}`,
      `**Neden ${isim}: **\t\t${collected.map(m => m.content).slice(4,5)}`
    ])
    .setTimestamp()
    .setFooter('Yılmaz Dev', message.guild.iconURL());
